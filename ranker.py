...
- train_neural(texts, labels, config) -> trains embedding-based neural ranker (Phase 2)
- compare_models(texts, labels) -> cross-validated results for TF-IDF, embeddings, and neural (if available)
...
# Neural ranker training (Phase 2)
def train_neural(texts: List[str], labels: List[str], config: dict = None):
    """Train the embedding-based neural ranker from Phase 2.

    Delegates to neural_ranker.train_ranker() and returns (metrics, exp_dir).
    """
    from neural_ranker import train_ranker
    return train_ranker(texts, labels, config=config)
...
def compare_models(texts: List[str], labels: List[str], embed_model_name: str = "all-MiniLM-L6-v2", cv: int = 5):
    ...
    # Neural ranker comparison (Phase 2)
    try:
        neural_metrics, _ = train_neural(texts, labels)
        results["neural"] = {
            "mean": neural_metrics.get("best_val_acc", float("nan")),
            "best_epoch": neural_metrics.get("best_epoch"),
            "best_val_loss": neural_metrics.get("best_val_loss"),
            "val_acc_history": neural_metrics.get("val_acc_history", []),
        }
    except Exception as e:
...
def predict_with_ranker(text: Union[str, List[str]], ranker_path: str = "ranker.pkl"):
    ...
    if typ == "tfidf":
        ...
        try:
            from neural_ranker import predict_proba as neural_predict_proba
            model_path = payload.get("model_path", "models/neural_ranker_best.pt")
            if len(texts) == 1:
                label, prob = neural_predict_proba(texts[0], model_path=model_path)
                return label, prob
            results = neural_predict_proba(texts, model_path=model_path)
            return results, None
        except Exception as e:
            return None, f"neural_error: {e}"
    else:
